...
main:
    ...
    main_loop:
        ...
        case_6:
            ...
            la $a0,SparseA  #printSparse(SparseA, mikosA)
            lw $a1,mikosA
            jal printSparse
...
printSparse:    #printSparse (int [] Sparse, int mikos)

    move $s0,$a0    #$s0 = Sparse (base register)
    move $s1,$a1    #$s1 = mikos
    li $s2,0    #i = 0

    printLoop:
        bge $s2,$s1,endPS   #if( i >= mikos)
